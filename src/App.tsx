      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        >
          <div className="flex items-center gap-2 text-[12px] pl-1" style={{ color: 'var(--ely-text-muted)' }}>
            ...
            {deskMode && (
              <span style={{ color: 'var(--ely-warning)' }}>· Modo escritorio</span>
            )}
            {showChatBar && ((wakeEnabled && wakeListening) || continuous) && (
              <span>· Escucha activa</span>
            )}
            {naturalTts && showChatBar && <span>· Voz neural</span>}
            {isAdmin && hasApiKey && (
              <span style={{ color: 'var(--ely-accent)' }}>· IA activa</span>
            )}
          </div>
        ...
        <div className="flex-1 flex min-h-0">
          <main
            className={`flex-1 flex flex-col min-w-0 ${
              page === 'productos' ? 'px-0 py-0' : 'px-6 py-5'
            }`}
          >
            <PageTransition page={page}>
                ...
                <>
                    ...
                    <div className="status-chip mt-2 mx-auto" key={statusLabel}>
                      {thinking || transcribing ? (
                        ...
                        <span
                          ...
                          style={{
                            background:
                              speaking || listening || wakeListening || deskMode
                                ? 'var(--ely-accent)'
                                : 'var(--ely-success)',
                          }}
                        />
                  ...
                  <div className="flex-1 flex items-center justify-center min-h-0">
                    <NetworkGlobe
                      ...
                      listening={listening || transcribing || wakeListening || deskMode}
                      size={320}
                      amplitude={amplitude}
                    />
                  </div>
              ...
              {page === 'productos' && canAccessPage(currentUser, 'productos') && (
                <ProductsPanel onSelectProduct={handleSelectProduct} />
              )}
              {page === 'registro-prensa' && canAccessPage(currentUser, 'registro-prensa') && (
                <RegistroPrensaPanel onSelectView={handleRegistroView} />
              )}
              {page === 'afq' && canAccessPage(currentUser, 'afq') && (
                <AfqPanel onSelectProduct={handleSelectProduct} />
              )}
              {page === 'cronograma' && canAccessPage(currentUser, 'cronograma') && (
                <CronogramaPanel />
              )}
              {page === 'config' && canAccessPage(currentUser, 'config') && (
                <div className="max-w-lg mx-auto w-full space-y-5 pt-2 overflow-y-auto pb-4">
                  ...
                  {isAdmin && (
                    <AiConfigPanel
                      hasApiKey={hasApiKey}
                      onHasApiKeyChange={setHasApiKey}
                      initialBaseUrl={cfgBaseUrl}
                      initialModel={cfgModel}
                      cfgLoaded={cfgLoaded}
                      isDesktop={isDesktop}
                    />
                  )}
                  {!isAdmin && (
                    <div
                      className="hud-glass p-4 text-[13px]"
                      style={{ color: 'var(--ely-text-muted)' }}
                    >
                      La configuración de usuarios y API keys solo está disponible para el
                      administrador.
                    </div>
                  )}
                  <div className="hud-glass p-5 space-y-4">
                    ...
                    <p
                      ...
                      style={{ color: 'var(--ely-text-muted)' }}
                    >
                      Al minimizar, ELYRA queda a un lado y sigue escuchando. Ctrl+Shift+E
                      oculta el sistema completo.
                    </p>
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        style={{
                          background: wakeEnabled ? 'var(--ely-accent)' : 'var(--ely-border)',
                          transition: 'background 0.25s var(--ely-ease)',
                        }}
                      >
                        <span
                          className={`ely-switch-thumb absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow ${
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="relative w-11 h-6 rounded-full"
                        style={{
                          background: continuous ? 'var(--ely-accent)' : 'var(--ely-border)',
                          transition: 'background 0.25s var(--ely-ease)',
                        }}
                      >
                        <span
              ...
              <div className="w-full max-w-xl mx-auto mt-auto pt-4">
                ...
                <div className="flex items-center gap-2 input-hud px-3 py-2">
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-full flex items-center justify-center disabled:opacity-30 transition-transform active:scale-95"
                    style={{ color: 'var(--ely-accent)' }}
                  >
                    {thinking ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
